document.addEventListener('DOMContentLoaded', () => {
  ...
  const swEnterSend = document.getElementById('swEnterSend');
  const swAutoScroll = document.getElementById('swAutoScroll');
  const swTimestamps = document.getElementById('swTimestamps');
  ...
  const pickShortcutsOverlay = document.getElementById('pickShortcutsOverlay');
  const pickShortcutsSheet = document.getElementById('pickShortcutsSheet');
  ...
  let lastRequest = null;          // { type:'chat', message, image } | { type:'image', prompt }
  ...
  function applyTheme() {
    ...
    document.documentElement.setAttribute('data-show-timestamps', showTimestamps ? '1' : '0');
  ...
  function savePrefs() {
    localStorage.setItem('eka_prefs', JSON.stringify({
      ...
      enterToSend, autoScrollEnabled, showTimestamps, compactMode
    }));
  }
  function applyPrefs() {
    const p = loadPrefs();
    ...
    enterToSend = p.enterToSend === undefined ? true : !!p.enterToSend;
    autoScrollEnabled = p.autoScrollEnabled === undefined ? true : !!p.autoScrollEnabled;
    showTimestamps = !!p.showTimestamps;
    compactMode = !!p.compactMode;
    ...
    swEnterSend.checked = enterToSend;
    swAutoScroll.checked = autoScrollEnabled;
    swTimestamps.checked = showTimestamps;
    swCompact.checked = compactMode;
  ...
  function startNewSession(withWelcome) {
    ...
    if (withWelcome) {
      ...
      showGreeting(profile && profile.name);
    } else {
      hideGreetingInstant();
    }
    updateScrollButton();
  ...
  function restoreLastSession() {
    ...
    if (found) {
      ...
      chat.innerHTML = '';
      hideGreetingInstant();
      lastRequest = null; lastRegenBtn = null;
  ...
  function updateScrollButton() {
    const shouldShow = !isNearBottom() && chat.scrollHeight > chat.clientHeight + 40;
    btnScrollBottom.hidden = false;
    btnScrollBottom.classList.toggle('show', shouldShow);
  }
  function scrollChatToBottom(force = false) {
    if (force || autoScrollEnabled) chat.scrollTop = chat.scrollHeight;
    updateScrollButton();
  }
  ...
  // CENTERED GREETING
  // ══════════════════════════════
  const greetingTypeText = document.getElementById('greetingTypeText');
  ...
  function typewriterLoop(phraseIdx, charIdx, deleting) {
    if (!greetingTypeText) return;
    const phrase = GREETING_PHRASES[phraseIdx];
    if (!deleting) {
      greetingTypeText.textContent = phrase.slice(0, charIdx);
      if (charIdx < phrase.length) {
        greetingTypeTimer = setTimeout(() => typewriterLoop(phraseIdx, charIdx + 1, false), 45);
      } else {
        greetingTypeTimer = setTimeout(() => typewriterLoop(phraseIdx, charIdx, true), 1400);
      }
    } else {
      greetingTypeText.textContent = phrase.slice(0, charIdx);
      if (charIdx > 0) {
        greetingTypeTimer = setTimeout(() => typewriterLoop(phraseIdx, charIdx - 1, true), 22);
      } else {
        const next = (phraseIdx + 1) % GREETING_PHRASES.length;
        greetingTypeTimer = setTimeout(() => typewriterLoop(next, 0, false), 300);
      }
    }
  }
  function startGreetingTyping() {
    stopGreetingTyping();
    if (greetingTypeText) greetingTypeText.textContent = '';
    greetingTypeTimer = setTimeout(() => typewriterLoop(0, 0, false), 900);
  }
  function stopGreetingTyping() {
    if (greetingTypeTimer) { clearTimeout(greetingTypeTimer); greetingTypeTimer = null; }
  }

  function showGreeting(name) {
    greetingName.textContent = (name && name !== 'Guest') ? name : 'there';
    chatGreeting.hidden = false;
    chatGreeting.classList.remove('hide');
    requestAnimationFrame(() => chatGreeting.classList.add('show'));
    startGreetingTyping();
  }
  function hideGreeting() {
    if (chatGreeting.hidden || !chatGreeting.classList.contains('show')) return;
    chatGreeting.classList.remove('show');
    chatGreeting.classList.add('hide');
    stopGreetingTyping();
    setTimeout(() => { chatGreeting.hidden = true; chatGreeting.classList.remove('hide'); }, 420);
  }
  function hideGreetingInstant() {
    chatGreeting.classList.remove('show', 'hide');
  ...
  function addBubble(text, who = 'bot', source = '', animate = false, imgData = null, skipLog = false, allowRegen = false) {
    ...
    const finalize = () => {
      ...
      if (who === 'bot') {
        ...
        if (allowRegen) {
          const regenBtn = document.createElement('button'); regenBtn.className = 'msg-action-btn'; regenBtn.title = 'Regenerate response';
          regenBtn.innerHTML = ICON_REGEN;
          regenBtn.addEventListener('click', () => regenerateResponse(regenBtn));
          actions.appendChild(regenBtn);
          attachRipple(regenBtn);
          if (lastRegenBtn && lastRegenBtn.isConnected) lastRegenBtn.remove();
          lastRegenBtn = regenBtn;
        }
  ...
  function addTyping() {
    ...
    chat.appendChild(t); scrollChatToBottom(true);
  }
  function removeTyping() { document.getElementById('mTypingRow')?.remove(); }

  // ── Image-generation "drawing" placeholder — glowing aura, orbiting particles, brush-stroke icon ──
    ...
    row.innerHTML = `
      <div class="bubble img-gen-bubble">
        <div class="img-gen-canvas">
          <div class="img-gen-aura"></div>
          <div class="img-gen-grid"></div>
          <div class="img-gen-shimmer"></div>
          <div class="img-gen-sweep"></div>
          <div class="img-gen-orbit"><span></span><span></span><span></span></div>
          <div class="img-gen-icon">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
              <path class="brush-path" d="M4 20l4-1 10-10a2.1 2.1 0 0 0-3-3L5 17l-1 3z"/>
              <path d="M13 7l4 4" opacity="0.7"/>
            </svg>
          </div>
        </div>
        <div class="img-gen-label"><span class="img-gen-dot"></span><span class="img-gen-text">Painting your image…</span></div>
      </div>`;
    chat.appendChild(row); scrollChatToBottom(true);
  }
  function removeImageGenerating() { document.getElementById('mImageGenRow')?.remove(); }

  function addImageBubble(dataUrl, prompt, skipLog = false, allowRegen = false) {
    if (!skipLog) {
    ...
    if (allowRegen) {
      const regenBtn = document.createElement('button'); regenBtn.className = 'msg-action-btn'; regenBtn.title = 'Regenerate image';
      regenBtn.innerHTML = ICON_REGEN;
      regenBtn.addEventListener('click', () => regenerateResponse(regenBtn));
      actions.appendChild(regenBtn);
      attachRipple(regenBtn);
  ...
  async function sendMessage(text) {
    ...
    if (imageGenEnabled && !attachedImage) {
      ...
      lastRequest = { type: 'image', prompt: cleaned };
      await runImageRequest(cleaned);
      return;
    }
    ...
    lastRequest = { type: 'chat', message: cleaned, image: imageToSend };
    await runChatRequest(lastRequest);
  }

  // ── Image generation request — used by both sendMessage() and regenerateResponse() ──
  async function runImageRequest(prompt) {
    addImageGenerating(prompt); isThinking = true; setStatus('thinking'); showStopButton();
    ...
    try {
      ...
      removeImageGenerating(); isThinking = false; setStatus('ready'); hideStopButton();

      if (res.image) {
        addImageBubble(res.image, prompt, false, true);
        vibrate(14);
      } else {
      ...
      removeImageGenerating(); isThinking = false; setStatus('ready'); hideStopButton();
      if (e && e.name === 'AbortError') addBubble('Image generation stopped.', 'bot');
      else addBubble('Something went wrong generating that image. Please try again.', 'bot');
    } finally {
      activeAbortController = null;
    }
  }

  // ── Chat request — used by both sendMessage() and regenerateResponse() ──
  async function runChatRequest(reqObj) {
    addTyping(); isThinking = true; setStatus('thinking'); showStopButton();
    activeAbortController = new AbortController();

    try {
      const body = {
        message: (reqObj.message || 'Please analyse this image.') + (PERSONA_TAGS[persona] || ''),
        history: chatHistory, wiki: webSearchEnabled, lang: currentLang
      };
      if (reqObj.image) body.image = reqObj.image;
      ...
      removeTyping(); isThinking = false; hideStopButton();
      ...
      setTimeout(() => {
        addBubble(res.reply, 'bot', srcLabel, true, null, false, true);
        setStatus('speaking');
        const plain = res.reply.replace(/(\*\*|__|[\*_`])/g,'').replace(/<[^>]*>/g,'').replace(/[^\p{L}\p{N}\s.,!?]/gu,'').trim();
        speak(plain, () => { setStatus('ready'); maybeShowFollowups(); });
      }, 180);
    } catch (e) {
      removeTyping(); isThinking = false; setStatus('ready'); hideStopButton();
      if (e && e.name === 'AbortError') addBubble('Response stopped.', 'bot');
      else addBubble('Something went wrong reaching Eka. Please try again.', 'bot');
    } finally {
      activeAbortController = null;
    }
  }

  // ── Regenerate: drop the last bot reply and re-run the same request ──
  async function regenerateResponse(triggerBtn) {
    if (isThinking || !lastRequest) return;
    vibrate(8);
    if (triggerBtn) triggerBtn.classList.add('regenerating');

    if (chatHistory.length && chatHistory[chatHistory.length - 1].role === 'assistant') chatHistory.pop();
    if (renderLog.length && renderLog[renderLog.length - 1].who === 'bot') renderLog.pop();
    const lastBotRow = [...chat.querySelectorAll('.row.bot')].pop();
    if (lastBotRow) lastBotRow.remove();
    lastRegenBtn = null;
    persistCurrentSession();

    if (lastRequest.type === 'image') await runImageRequest(lastRequest.prompt);
    else await runChatRequest(lastRequest);
  }

  // ── Stop responding: aborts the in-flight fetch and swaps the send button back ──
  function showStopButton() {
    mSend.classList.add('stop-mode');
    mSend.innerHTML = ICON_STOP_SEND;
    mSend.setAttribute('aria-label', 'Stop responding');
    mSend.dataset.mode = 'stop';
  }
  function hideStopButton() {
    mSend.classList.remove('stop-mode');
    mSend.innerHTML = ORIGINAL_SEND_ICON;
    mSend.setAttribute('aria-label', 'Send message');
    mSend.dataset.mode = 'send';
  }
  ...
  mSend.addEventListener('click', () => {
    if (mSend.dataset.mode === 'stop') stopResponding();
    else sendMessage(mMsg.value);
  });
  mMsg.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter' || e.shiftKey) return;
    if (enterToSend) { e.preventDefault(); sendMessage(mMsg.value); }
    // when Enter-to-send is off, Ctrl/Cmd+Enter still sends; plain Enter inserts a newline
    else if (e.ctrlKey || e.metaKey) { e.preventDefault(); sendMessage(mMsg.value); }
  });
  ...
  function enterApp(profile) {
    ...
    applyPrefs();
    updateHistoryCount();
    if (chat.querySelectorAll('.row').length === 0) { if (!restoreLastSession()) startNewSession(true); }
    attachRippleAll();
  }
  ...
  swEnterSend.addEventListener('change', () => { enterToSend = swEnterSend.checked; savePrefs(); vibrate(6); });
  swAutoScroll.addEventListener('change', () => { autoScrollEnabled = swAutoScroll.checked; savePrefs(); vibrate(6); if (autoScrollEnabled) scrollChatToBottom(true); });
  swTimestamps.addEventListener('change', () => { showTimestamps = swTimestamps.checked; savePrefs(); applyTheme(); vibrate(6); });
  swCompact.addEventListener('change', () => { compactMode = swCompact.checked; savePrefs(); applyTheme(); vibrate(6); });

  // ── About: keyboard shortcuts + feedback ──
  btnKeyboardShortcuts.addEventListener('click', () => { pickShortcutsOverlay.classList.add('show'); attachRippleAll(); });
  pickShortcutsOverlay.addEventListener('click', (e) => { if (e.target === pickShortcutsOverlay) pickShortcutsOverlay.classList.remove('show'); });
  pickShortcutsSheet.addEventListener('click', (e) => e.stopPropagation());
  btnSendFeedback.addEventListener('click', () => { showToast('Use the 👍 / 👎 buttons on any reply to send feedback'); vibrate(6); });
